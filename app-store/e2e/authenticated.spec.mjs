...
test('dedicated accounts remain isolated and restore across sessions',async({request,browser})=>{
  ...
  try{
    await putState(request,a.access_token,{appVersion:'30.1.27',storage:{'wgp-v15-e2e-marker':JSON.stringify(markerA)}});
    await putState(request,b.access_token,{appVersion:'30.1.27',storage:{'wgp-v15-e2e-marker':JSON.stringify(markerB)}});
    ...
    await putState(request,restoreA.access_token,beforeA.state||{appVersion:'30.1.27',storage:{}});
    await putState(request,restoreB.access_token,beforeB.state||{appVersion:'30.1.27',storage:{}});
  }
});
