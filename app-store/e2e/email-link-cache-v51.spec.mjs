...
test.beforeAll(async()=>{
 server=createServer(async(req,res)=>{
  ...
  if(url.pathname==='/legacy-worker.js'){
   ...
   return res.end(`self.addEventListener('install',e=>e.waitUntil(self.skipWaiting()));self.addEventListener('activate',e=>e.waitUntil(self.clients.claim()));self.addEventListener('fetch',e=>{if(e.request.mode!=='navigate')return;e.respondWith(caches.open('legacy-privacy52-fixture').then(async c=>(await c.match(e.request,{ignoreSearch:true}))||fetch(e.request)))});`);
  }
...
for(const width of [390,1440])test.describe(`${width}px confirmation with offline cache`,()=>{
 ...
 test('cached redirected pages load; signup and reset links bypass cache and finish their own flow',async({page},info)=>{
  ...
  expect(cachedUrls.some(url=>/[?&](?:code|auth)=/.test(url))).toBe(false);expect(cacheName).toContain('privacy52');expect(errors).toEqual([]);
 });
});
...
test('new direct-shell links still open when an older worker has the broken folder response',async({page})=>{
 ...
 await page.evaluate(async()=>{
  ...
  const cache=await caches.open('legacy-privacy52-fixture');await cache.put('/work-gym-planner/',await fetch('/redirected-shell'));await cache.put('/work-gym-planner/shell.html',await fetch('/work-gym-planner/shell.html'));
