...
html=rewrite(
  ...
  "h=h.replace('<head>','<head><base href=\"../work-gym-planner-v15/\"><scr'+'ipt defer src=\"../native/native-bridge.js?v=30.1.31\"></scr'+'ipt><scr'+'ipt defer src=\"../shared/observability.js?v=30.1.31-calendar54\"></scr'+'ipt>');",
  'load native bridge first'
);
html=html.replace("'pwa-patch.js',",'');
html=html.replace("h=h.replace('<link rel=\"manifest\" href=\"./manifest.webmanifest\">','<link rel=\"manifest\" href=\"../work-gym-planner/manifest.webmanifest?v=30.1.31\">');","h=h.replace(/<link rel=\"manifest\"[^>]*>/g,'');");
...
console.log(`Built Work + Workout 30.1.31 native web bundle: ${out}`);
