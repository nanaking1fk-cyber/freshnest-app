...
for(const file of workers){
 test(`${file}: cached redirected pages are safe to replay and retain security headers`,async()=>{
  ...
  assert.equal(response.redirected,false);assert.equal(response.status,200);assert.equal(await response.text(),'offline planner');assert.equal(response.headers.get('content-security-policy'),"default-src 'self'");assert.equal(h.calls.network.length,0);assert.ok(h.calls.opens.every(name=>name.endsWith('-apple69')));
 });
...
test('new emails use the direct shell and the fixed worker is refreshed',()=>{
 ...
 for(const file of ['work-gym-planner/boot.js','work-gym-planner/shell.html','work-gym-planner/index.html',...workers])assert.ok(read(file).includes('30.1.31-apple69'));
});
