...
function harness({purpose='recovery',query='?auth=recovery&code=fixture',session=null,failure=false}={}){
 ...
 vm.createContext(c);
 vm.runInContext(source.slice(0,source.indexOf(' loadSession();if(!A.session)'))+'A.testCallback=consumeAuthRedirect;A.testLoadSession=loadSession;A.testRefreshSession=refreshSession;})(window.WGC18);',c);
