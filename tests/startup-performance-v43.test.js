...
for(const file of ['work-gym-planner/sw.js','work-gym-planner-v16/sw.js']){
  test(file+' serves cached scripts without waiting for the network',async()=>{
    ...
    vm.runInNewContext(read(file),{self:{addEventListener:(name,handler)=>listeners[name]=handler},caches:{open:async()=>({match:async()=>cached})},fetch:async()=>{fetched++;throw Error('must not fetch')},URL,location:{origin:'https://example.test'}});
    let response;
  ...
  test(file+' downloads optional tools on demand and leaves API traffic uncached',async()=>{
    ...
    vm.runInNewContext(read(file),{self:{addEventListener:(name,handler)=>listeners[name]=handler},caches:{open:async()=>({match:async()=>null,put:async request=>puts.push(request.url)})},fetch:async()=>{fetched++;return new Response('tool')},URL,location:{origin:'https://example.test'}});
    let response;
