...
(function(A){
 ...
 body.innerHTML=`${A.passwordRecovery?'<div class="accountUnavailable"><b>Choose a new password</b><p>Use 12+ characters with uppercase, lowercase, a number and a symbol.</p><label>New password<input id="recoveryNewPassword" type="password" minlength="12" autocomplete="new-password"></label><button id="saveRecoveredPassword" class="primary wideBtn">Set new password</button></div>':''}<div class="signedAccount"><div class="accountIdentity"><span>${esc((accountEmail()||'?')[0].toUpperCase())}</span><div><b>${esc(accountEmail()||'Signed in')}</b><small>Last sync: ${esc(when)}</small></div><button id="signOutAccount">Sign out</button></div>${A.healthConsentPanelHTML?.()||''}<div class="accountActions"><button id="migrateDevice" class="primary"><b>Sync this device</b><small>Upload only this account's planner, training and nutrition records</small></button><button id="restoreAccount"><b>Restore from account</b><small>Replace this device with the private cloud copy</small></button><button id="syncAccount"><b>Sync now</b><small>Update your account with this device</small></button><button id="startOnboardingAccount"><b>Edit adaptive plan</b><small>Update your work, workouts, recovery and nutrition preferences</small></button></div><div class="dangerZone"><button id="deleteCloudAccount" class="danger">Delete account permanently</button></div></div>`;
