...
(function(A){
 ...
 // Email proof is independent of the browser that requested the reset. Keep
 // it out of storage/history and exchange it only after a deliberate tap.
 let recoveryEmailProof=new URLSearchParams(location.hash.slice(1)).get('recovery_token');
 if(recoveryEmailProof)window.history.replaceState(null,'',location.pathname+'?auth=recovery');
 ...
 let accountLoad=null,sessionRefresh=null,configLoad=null,accountStartup=null,authMode='signin';
 const BASELINE_PREFIX='wgc-v44-cloud-revision:',BASELINE_STATE_PREFIX='wgc-v80-cloud-fingerprint:',PROTECTED_PREFIX='wgc-v44-protected-copy:';
 ...
 // A same-account device copy is safe to open while a genuine sync conflict
 // waits for review. Cross-account copies remain locked and onboarding remains
 // blocked, so opening Home here cannot expose or replace another user's data.
 A.canOpenHome=()=>!A.deviceStorageBlocked&&!!A.session&&A.accountState==='choice'&&localStorage.getItem(OWNER_KEY)===A.session?.user?.id&&meaningfulState(A.captureLocalState?.()||captureLocalState());
 ...
 function loadConfig(){
  ...
  A.apiBase=absoluteApiBase();A.config.loaded=false;A.config.error=null;
  renderAccountUI();
  configLoad=(async()=>{
   try{
    const j=await raw(A.api('config'));
    if(typeof j.cloudConfigured!=='boolean')throw Error('Incomplete account connection response');
    A.config={...A.config,...j,loaded:true,error:null};
   }catch(e){A.config.loaded=true;A.config.error=e.message||'Connection failed'}
   finally{configLoad=null;renderAccountUI()}
   return A.config;
  })();
  return configLoad;
 }
 function selectAuthMode(){
  $$('[data-auth-tab]').forEach(button=>button.classList.toggle('active',button.dataset.authTab===authMode));
  $('#signinPane')?.classList.toggle('hidden',authMode!=='signin');
  $('#signupPane')?.classList.toggle('hidden',authMode!=='signup');
 }
 function finishSignInToHome(){
  A.signInCompleting=false;
  status('');
  closeModal('accountDialog');
  try{
   if(typeof window.page==='function')window.page('home');
   else document.querySelector('.bottomNav [data-page="home"]')?.click();
 ...
 A.pushState=async function({quiet=false}={}){if(!A.session)return false;if(!A.cloudStateReady){if(quiet)return false;await A.resumeAccount?.();if(!A.cloudStateReady)return false}A.assertCloudReady();if(!await A.ensureHealthConsent?.({interactive:!quiet,purpose:'account_cloud_sync'}))return false;let uid=A.session?.user?.id,state=A.captureLocalState(),j=await A.authedFetch('state',{method:'PUT',body:JSON.stringify({state,baseUpdatedAt:A.cloudRevision})});A.acceptCloudRevision(j.updatedAt);rememberCloudFingerprint(uid,state);localStorage.setItem(LAST_SYNC_KEY,j.updatedAt||new Date().toISOString());if(!quiet){status('Your account is synced.');toast('Account sync complete')}renderAccountUI();return true};
 ...
 function cloudFingerprint(uid){try{return localStorage.getItem(BASELINE_STATE_PREFIX+uid)||null}catch{return null}}
 function rememberCloudFingerprint(uid,state){if(!uid)return;try{localStorage.setItem(BASELINE_STATE_PREFIX+uid,stateFingerprint(state||{storage:{}}))}catch{}}
 A.rememberCloudFingerprint=rememberCloudFingerprint;
 ...
 async function afterAuth({forceCloud=false}={}){
  ...
  accountLoad=(async()=>{
   ...
   if(!meaningfulState(A.captureLocalState?.()||captureLocalState())&&meaningfulState(cached))restoreCloudState(cached,{snapshot:false});
    ...
    if(remote.state){
     const localFingerprint=stateFingerprint(local),remoteFingerprint=stateFingerprint(remote.state),knownCloudFingerprint=cloudFingerprint(uid);
     if(!forceCloud&&meaningfulState(local)&&localFingerprint!==remoteFingerprint&&baseline!==remote.updatedAt){
      // Resolve only when the stored sync fingerprint proves which side changed.
      // Older installations without that proof keep both copies and let the
      // user continue into Home with cloud writes paused until they review it.
      if(knownCloudFingerprint&&localFingerprint===knownCloudFingerprint){
       protectCopy(uid,local);stashForUser(uid);restoreCloudState(remote.state);rememberCloudFingerprint(uid,remote.state);finishAccountLoad(remote);status('Your latest saved plan is ready.');return{ok:true,autoResolved:'cloud'};
      }else if(knownCloudFingerprint&&remoteFingerprint===knownCloudFingerprint){
       protectCopy(uid,remote.state);finishAccountLoad(remote);A.queueSync();status('Your phone has the newer plan. Backup will update safely.');return{ok:true,local:true};
      }else{
       protectCopy(uid,local);setAccountState('choice');status('Signed in. Your phone plan is open; review backup when convenient.');return{ok:true,local:true,needsReview:true};
      }
     }
     if(forceCloud||!meaningfulState(local)||baseline!==remote.updatedAt){
      protectCopy(uid,local);stashForUser(uid);restoreCloudState(remote.state);rememberCloudFingerprint(uid,remote.state);
     }
     else if(localFingerprint===remoteFingerprint)rememberCloudFingerprint(uid,remote.state);
    }
    else if(!meaningfulState(local))rememberCloudFingerprint(uid,null);
 ...
 function accountSafetyHTML(){
  ...
  return '<div class="accountUnavailable" id="accountRestoreGuard"><b>'+(checking?'Checking your backup…':local?'Using this phone':choice?'Which plan should this phone use?':'Your saved plan is protected')+'</b><p>'+(checking?'This usually takes only a moment.':local?'Your plan stays on this phone. Your saved online plan will not change.':choice?'We found one plan on this phone and another in your account. Both are safe.':'We could not load your saved plan, and nothing was replaced.')+'</p>'+(checking?'':'<button id="loadSavedAccount" class="primary wideBtn">'+(choice?'Use my saved plan':'Try loading saved plan')+'</button><button id="useDeviceOnly" class="wideBtn">'+(choice?'Keep this phone’s plan for now':'Continue on this phone')+'</button>')+'</div>';
 }
 function bindAccountSafety(){
  ...
  $('#reviewAccountSync')?.addEventListener('click',()=>{const section=$('#accountDataSection');if(section)section.open=true;$('#accountRestoreGuard')?.scrollIntoView?.({block:'nearest',behavior:'smooth'})});
 }
 ...
 async function consumeAuthRedirect(){
  ...
  if(recoveryEmailProof){A.authLinkError=null;A.recoveryLinkPending=true;openAccount('signin');return true}
  ...
  if(!verifier){openAccount('signin');showAuthLinkError(purpose==='recovery'?'This older reset link cannot be completed here. Request a new reset email below.':'This link opened in a different browser. Sign in with the email and password you used to create your account to continue.');return true}
  try{
 ...
 async function verifyRecoveryEmail(){
  if(A.recoveryVerifying||!recoveryEmailProof)return;
  A.recoveryVerifying=true;renderAccountUI();status('Checking your reset link…');
  try{
   const session=await raw(`${A.config.supabaseUrl}/auth/v1/verify`,{method:'POST',headers:{apikey:A.config.supabaseAnonKey,'Content-Type':'application/json'},body:JSON.stringify({token_hash:recoveryEmailProof,type:'recovery'})});
   if(!session?.access_token||!session?.refresh_token||!session?.user?.id)throw Error('Invalid recovery response');
   sessionStorage.setItem(RECOVERY_KEY,'1');A.passwordRecovery=true;session.passwordRecoveryPending=true;
   recoveryEmailProof=null;A.recoveryLinkPending=false;
   saveSession(session);openAccount('signin');status('Choose a new password to finish recovery.');
  }catch(error){
   // Network failures can be retried. A rejected/consumed email needs a new link.
   if(error?.status>=400&&error?.status<500){recoveryEmailProof=null;A.recoveryLinkPending=false;showAuthLinkError('This reset link has expired or was already used. Request a new reset email.')}
   else status('We couldn’t check your link. Check your connection and try again.',true);
  }finally{A.recoveryVerifying=false;renderAccountUI()}
 }
 function renderRecoveryLink(body){
  if(!$('#verifyRecoveryEmail')){
   body.innerHTML='<div class="authPane"><b>Reset your password</b><p>Continue to choose a new password for the account that received this email.</p><button id="verifyRecoveryEmail" class="primary wideBtn">Continue to reset password</button></div>';
   $('#verifyRecoveryEmail').onclick=verifyRecoveryEmail;
  }
  $('#verifyRecoveryEmail').disabled=!!A.recoveryVerifying;
 }
 ...
 function renderAuthLinkError(body){
  ...
  $('#requestFreshReset').onsubmit=async event=>{event.preventDefault();if(A.authBusy)return;A.authBusy=true;const button=$('#freshResetButton');button.disabled=true;try{await recover($('#resetLinkEmail').value.trim());status('Reset email sent. Open the newest email to choose your password.')}catch(e){status(e.message,true)}finally{A.authBusy=false;button.disabled=false}};
 ...
 function clearDeletedAccountLocally(uid){
  ...
  for(const key of [cacheKey(uid),PROTECTED_PREFIX+uid,BASELINE_PREFIX+uid,BASELINE_STATE_PREFIX+uid,'wgc-health-consent-v35:'+uid,'ww-workpay-v58:'+uid])localStorage.removeItem(key);
  clearRecoveryFlag()
 }
 ...
 function renderAccountUI(){let signed=!!A.session?.access_token,body=$('#accountBody'),chip=$('#accountChip'),needsReview=signed&&A.accountState==='choice';$('#signOutQuick')?.remove();if(chip){chip.textContent=signed?(accountEmail()||'Account'):'Sign in';chip.classList.toggle('signed',signed);chip.classList.toggle('needsReview',needsReview)}const menuText=$('#accountMenuText');if(menuText)menuText.textContent=needsReview?'Signed in · saved plan needs a choice':'Plan, data, privacy and sign out';if(!body)return;if(!A.config.loaded){body.innerHTML='<p class="muted" role="status">Connecting securely…</p>';return}
 if(A.config.error||!A.config.cloudConfigured){
  // Background UI refreshes must not replace a retry button during a tap.
  if($('#retryAccountConnection'))return;
 ...
 const title=$('#accountTitle');if(title)title.textContent=A.passwordRecovery?'Reset your password':A.signInCompleting?'Opening Work + Workout':signed?'Account':'Your Work + Workout account';
 ...
 if(A.recoveryLinkPending){renderRecoveryLink(body);return}
 if(signed&&A.signInCompleting){body.innerHTML='<div class="accountUnavailable accountSignInTransition" role="status" aria-live="polite"><b>Opening your home…</b><p>Loading your saved plan safely.</p></div>';return}
 if(signed&&A.passwordRecovery&&$('#recoveryEmail')?.value===accountEmail())return;
 if(!signed&&$('#signinPane')){selectAuthMode();return}
 if(!signed){body.innerHTML=`<div class="authTabs"><button type="button" data-auth-tab="signin" class="active">Sign in</button><button type="button" data-auth-tab="signup">Create account</button></div><form id="signinPane" class="authPane" autocomplete="on"><label>Email<input id="loginEmail" name="username" type="email" inputmode="email" autocomplete="username" autocapitalize="none" spellcheck="false" required></label><label>Password<input id="loginPassword" name="password" type="password" autocomplete="current-password" required></label><button id="loginBtn" type="submit" class="primary wideBtn">Sign in</button><button id="recoverBtn" type="button" class="linkBtn">Forgot password?</button></form><form id="signupPane" class="authPane hidden" autocomplete="on"><label>Name<input id="signupName" name="name" autocomplete="name" required maxlength="80"></label><label>Email<input id="signupEmail" name="username" type="email" inputmode="email" autocomplete="username" autocapitalize="none" spellcheck="false" required></label><label>Password<input id="signupPassword" name="new-password" type="password" minlength="12" autocomplete="new-password" required></label><small>Use 12+ characters with uppercase, lowercase, a number and a symbol.</small><button id="signupBtn" type="submit" class="primary wideBtn">Create account</button><p class="muted">Next: choose your privacy settings, then build your plan. Cloud backup and AI features are optional.</p></form>`;$$('[data-auth-tab]').forEach(b=>b.onclick=()=>{authMode=b.dataset.authTab;selectAuthMode()});selectAuthMode();$('#signinPane').onsubmit=async event=>{event.preventDefault();if(A.authBusy)return;let email=$('#loginEmail'),password=$('#loginPassword');if(!email.checkValidity()||!password.value)return status('Enter a valid email and password.',true);A.authBusy=true;A.signInCompleting=true;status('Signing in…');try{await signIn(email.value.trim(),password.value);if(A.canStartOnboarding()||A.canOpenHome())finishSignInToHome();else{A.signInCompleting=false;renderAccountUI()}toast('Signed in')}catch(e){A.signInCompleting=false;renderAccountUI();status(e.message,true)}finally{A.authBusy=false}};$('#signupPane').onsubmit=async event=>{event.preventDefault();if(A.authBusy)return;let name=$('#signupName'),email=$('#signupEmail'),password=$('#signupPassword');if(!name.value.trim())return status('Enter your name.',true);if(!email.checkValidity())return status('Enter a valid email address.',true);if(!passwordStrong(password.value))return status('Use 12+ characters with uppercase, lowercase, a number and a symbol.',true);A.authBusy=true;status('Creating account…');try{let j=await signUp(name.value.trim(),email.value.trim(),password.value);if(!j.access_token)status('Account created. Open the confirmation email in this browser to finish securely.');else{closeModal('accountDialog');toast('Account created')}}catch(e){status(e.message,true)}finally{A.authBusy=false}};let recoverButton=$('#recoverBtn');runRecoveryCountdown(recoverButton);recoverButton.onclick=async()=>{let email=$('#loginEmail');if(A.authBusy||recoveryCooldown())return runRecoveryCountdown(recoverButton);if(!email.checkValidity())return status('Enter a valid email first.',true);A.authBusy=true;recoverButton.disabled=true;recoverButton.textContent='Sending reset email…';try{await recover(email.value.trim());status('Password reset email sent. Open the newest email to choose your password.')}catch(e){status(e.message,true)}finally{A.authBusy=false;runRecoveryCountdown(recoverButton)}};return}
 let last=localStorage.getItem(LAST_SYNC_KEY),when=last?new Date(last).toLocaleString():'Not backed up yet';
 const syncNeedsReview=A.accountState==='choice',syncLabel=A.cloudStateReady?'Backup ready':A.accountState==='local'?'Saved on this phone':syncNeedsReview?'Choose a saved plan':A.accountState==='checking'?'Checking backup…':'Backup needs attention';
 const syncNotice=syncNeedsReview?'<button id="reviewAccountSync" type="button" class="accountSyncNotice"><span><b>Choose which plan to use</b><small>Your phone plan is open. Both copies are protected.</small></span><i>Review</i></button>':'';
 const syncTools=A.cloudStateReady?`<div class="accountActions accountPrimarySync"><button id="syncAccount" class="primary"><b>Back up now</b><small>Save this phone’s latest changes</small></button></div><details class="accountAdvanced"><summary>More backup options <i>⌄</i></summary><div class="accountActions"><button id="restoreAccount"><b>Restore saved plan</b><small>Replace this phone’s plan after confirmation</small></button><button id="migrateDevice"><b>Use this phone as saved plan</b><small>Make this phone’s current plan the account copy</small></button></div></details>`:'';
 body.innerHTML=`${A.passwordRecovery?`<form id="recoveryPasswordForm" class="accountUnavailable authPane" autocomplete="on"><b>Choose a new password</b><p>Use 12+ characters with uppercase, lowercase, a number and a symbol.</p><label>Email<input id="recoveryEmail" name="username" type="email" value="${esc(accountEmail())}" autocomplete="username" readonly></label><label>New password<input id="recoveryNewPassword" name="new-password" type="password" minlength="12" autocomplete="new-password" required></label><button id="saveRecoveredPassword" type="submit" class="primary wideBtn">Set new password</button></form>`:''}<div class="signedAccount"><div class="accountIdentity"><span>${esc((accountEmail()||'?')[0].toUpperCase())}</span><div><b>${esc(accountEmail()||'Signed in')}</b><small>${esc(syncLabel)}${A.cloudStateReady?' · '+esc(when):''}</small></div></div>${syncNotice}<div class="accountMenu"><details id="accountPlanSection" class="accountMenuSection"><summary><span>Plan &amp; preferences<small>Work, workouts, recovery and nutrition</small></span><i>⌄</i></summary><div class="accountMenuBody"><div class="accountActions"><button id="startOnboardingAccount" type="button"><b>Review my plan</b><small>Update work and training preferences</small></button></div></div></details><details id="accountDataSection" class="accountMenuSection"><summary><span>Data &amp; sync<small>${esc(syncLabel)}</small></span><i>⌄</i></summary><div class="accountMenuBody">${accountSafetyHTML()}${syncTools}</div></details><details class="accountMenuSection"><summary><span>Privacy &amp; account<small>Your choices and account controls</small></span><i>⌄</i></summary><div class="accountMenuBody">${A.healthConsentPanelHTML?.()||''}<div class="dangerZone"><b>Delete account</b><p>Permanently remove your login and saved account data.</p><button id="deleteCloudAccount" class="danger">Delete account permanently</button></div></div></details></div><footer class="accountSessionActionsV71" aria-label="Session actions"><button id="signOutAccount" type="button">Sign out</button></footer></div>`;
 ...
 $('#migrateDevice')?.addEventListener('click',async()=>{if(!confirm(`Use this phone's ${localDataCount()} planner records as your saved account plan?`))return;status('Backing up this phone…');try{await A.pushState()}catch(e){status(e.message,true)}});
 $('#restoreAccount')?.addEventListener('click',()=>accountAction('restore'));
 $('#syncAccount')?.addEventListener('click',async()=>{status('Backing up…');try{await A.pushState()}catch(e){status(e.message,true)}});
 $('#startOnboardingAccount')?.addEventListener('click',()=>{closeModal('accountDialog');window.WGC18?.openOnboarding?.()});
 $('#signOutAccount')?.addEventListener('click',signOut);
 $('#deleteCloudAccount')?.addEventListener('click',()=>accountAction('delete'));
 }
 function openAccount(mode='signin'){if(mode==='signin'||mode==='signup')authMode=mode;renderAccountUI();if(location.protocol==='file:'){let body=$('#accountBody');if(body)body.innerHTML='<div class="accountUnavailable"><b>This is a local preview.</b><p>Accounts and private cloud sync only work on the secure live website.</p><a class="primary wideBtn accountLiveLink" href="https://www.workandworkout.com/">Open the secure website</a></div>';openModal('accountDialog');return}openModal('accountDialog');selectAuthMode();if(A.config.error)initializeAccount()}
 A.openAccount=openAccount;A.renderAccountUI=renderAccountUI;
 function injectUI(){let cards=$('#page-more .menuCards');if(cards&&!$('#openAccountV18')){let b=document.createElement('button');b.id='openAccountV18';b.innerHTML='<span>👤</span><div><b>Account & sync</b><small id="accountMenuText">Plan, data, privacy and sign out</small></div><i>›</i>';cards.insertBefore(b,cards.firstChild);b.onclick=()=>openAccount('signin')}let home=$('#todayDashboard');if(home&&!$('#accountChip')){let chip=document.createElement('button');chip.id='accountChip';chip.className='accountChip';chip.onclick=()=>openAccount('signin');home.parentElement?.insertBefore(chip,home)}$('#signOutQuick')?.remove();if(!$('#accountDialog'))document.body.insertAdjacentHTML('beforeend',`<div id="accountDialog" class="modal" role="dialog" aria-modal="true" aria-labelledby="accountTitle"><div class="sheet largeSheet premiumAccountSheet"><div class="sheetHandle"></div><div class="sheetHead"><h2 id="accountTitle">Account</h2><button data-close="accountDialog">Done</button></div><div id="accountBody"></div><p id="accountStatus" class="statusText"></p></div></div>`);let close=$('#accountDialog [data-close]');if(close)close.onclick=()=>closeModal('accountDialog');$('#accountDialog')?.addEventListener('click',e=>{if(e.target.id==='accountDialog')closeModal('accountDialog')});renderAccountUI()}
 function hookSync(){for(const name of ['saveProfileObj','saveNutritionObj','saveHistory','saveDrafts','saveOverrides','saveSnapshots','saveBodyLog','saveDiary','mergeHealthDay','saveBMonth']){let fn=window[name];if(typeof fn!=='function'||fn.__wgc18sync)continue;let wrapped=function(...args){let r=fn.apply(this,args);Promise.resolve(r).finally(()=>A.queueSync());return r};wrapped.__wgc18sync=true;window[name]=wrapped}}
 function initializeAccount(){
  if(accountStartup)return accountStartup;
  accountStartup=loadConfig().then(async()=>{
    // Keep callback codes and PKCE verifiers intact until a secure connection is ready.
    if(A.config.error||!A.config.cloudConfigured)return;
    ...
    renderAccountUI();
  }).finally(()=>{accountStartup=null});
  return accountStartup;
 }
 loadSession();if(!A.session)lockPlannerForLoggedOut();injectUI();hookSync();initializeAccount();
 window.addEventListener('online',()=>{if(A.config.error)initializeAccount()});
