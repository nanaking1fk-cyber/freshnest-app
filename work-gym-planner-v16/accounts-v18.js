...
(function(A){
 ...
 async function signUp(name,email,password){if(!A.config.cloudConfigured)throw Error('Account server is not configured yet.');let target=encodeURIComponent(authRedirectUrl()),j=await raw(`${A.config.supabaseUrl}/auth/v1/signup?redirect_to=${target}`,{method:'POST',headers:{apikey:A.config.supabaseAnonKey,'Content-Type':'application/json'},body:JSON.stringify({email,password,data:{display_name:name||''}})});if(j.access_token){saveSession(j);await afterAuth()}else lockPlannerForLoggedOut();return j}
 async function recover(email){if(!A.config.cloudConfigured)throw Error('Account server is not configured yet.');let target=encodeURIComponent(authRedirectUrl());await raw(`${A.config.supabaseUrl}/auth/v1/recover?redirect_to=${target}`,{method:'POST',headers:{apikey:A.config.supabaseAnonKey,'Content-Type':'application/json'},body:JSON.stringify({email})})}
 ...
 async function consumeAuthRedirect(){
  ...
  const params=new URLSearchParams(location.hash.slice(1)),access=params.get('access_token'),refresh=params.get('refresh_token');
  if(!access||!refresh)return false;
  try{
   const user=await raw(`${A.config.supabaseUrl}/auth/v1/user`,{headers:{apikey:A.config.supabaseAnonKey,Authorization:`Bearer ${access}`}});
   const expiresIn=+params.get('expires_in')||3600,expiresAt=+params.get('expires_at')||Math.floor(Date.now()/1000)+expiresIn;
   history.replaceState(null,'',location.pathname+location.search);
   saveSession({access_token:access,refresh_token:refresh,expires_in:expiresIn,expires_at:expiresAt,token_type:params.get('token_type')||'bearer',user});
   await afterAuth();
   toast('Email confirmed. Your private plan is ready to build.');
   return true
  }catch(e){history.replaceState(null,'',location.pathname+location.search);status('The confirmation link could not be completed. Sign in with your confirmed email.',true);return false}
 }
