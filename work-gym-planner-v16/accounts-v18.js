...
(function(A){
 ...
 function renderAccountUI(){let signed=!!A.session?.access_token,body=$('#accountBody'),chip=$('#accountChip');$('#signOutQuick')?.remove();if(chip){chip.textContent=signed?(accountEmail()||'Account'):'Sign in';chip.classList.toggle('signed',signed)}if(!body)return;if(!A.config.loaded){body.innerHTML='<p class="muted" role="status">Connecting securely…</p>';return}
 ...
 const title=$('#accountTitle');if(title)title.textContent=A.passwordRecovery?'Reset your password':A.signInCompleting?'Opening Work + Workout':'Your Work + Workout account';
