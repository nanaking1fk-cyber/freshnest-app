...
(function(A){
 ...
 async function afterAuth({forceCloud=false}={}){
  ...
  accountLoad=(async()=>{
   ...
   if(!meaningfulState(A.captureLocalState?.()||captureLocalState())&&meaningfulState(cached))restoreCloudState(cached,{snapshot:false});
    const consent=await A.ensureHealthConsent?.({interactive:true,purpose:'account_cloud_sync'});
 ...
 loadSession();if(!A.session)lockPlannerForLoggedOut();injectUI();hookSync();loadConfig().then(async()=>{await accountModulesReady;if(await consumeAuthRedirect())return;if(A.session&&sessionExpired())await refreshSession().catch(()=>{});if(!A.session)lockPlannerForLoggedOut();else await afterAuth();renderAccountUI()});document.addEventListener('DOMContentLoaded',()=>setTimeout(()=>{injectUI();hookSync();renderAccountUI()},80));
})(window.WGC18);
