...
(function(A){
 ...
 async function recover(email){if(!A.config.cloudConfigured)throw Error('Account server is not configured yet.');let target=encodeURIComponent(authRedirectUrl('recovery')),challenge=await beginPkce('recovery');await raw(`${A.config.supabaseUrl}/auth/v1/recover?redirect_to=${target}`,{method:'POST',headers:{apikey:A.config.supabaseAnonKey,'Content-Type':'application/json'},body:JSON.stringify({email,code_challenge:challenge,code_challenge_method:'s256'})})}
 ...
 A.pullState=async function({reload=true}={}){if(typeof A.ensureHealthConsent!=='function'||!await A.ensureHealthConsent({interactive:true,purpose:'account_cloud_sync'}))throw Error('Account restore remains off. Your planner is still available on this device.');let j=await A.authedFetch('state');if(!j.state)throw Error('This account does not have saved app data yet.');let count=restoreCloudState(j.state);localStorage.setItem(LAST_SYNC_KEY,j.updatedAt||new Date().toISOString());status(`Restored ${count} records from your account.`);if(reload)setTimeout(()=>location.reload(),500);else renderAll?.();return count};
 ...
 async function afterAuth(){
  ...
  let remote=null,mayProcess=false;try{mayProcess=!!(await A.ensureHealthConsent?.({interactive:false,purpose:'account_cloud_sync'}))}catch{}
  if(mayProcess)try{remote=await A.authedFetch('state')}catch{}
  const owner=localStorage.getItem(OWNER_KEY),hasVisibleData=localDataCount()>0;
