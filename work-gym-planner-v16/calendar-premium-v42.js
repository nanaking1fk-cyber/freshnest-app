...
(function calendarPremiumV42(window){
  ...
  function sourceById(sourceId){return sources().find(function(source){return source.id===sourceId})||firstSource()}
  function shiftVariants(sourceId){return Core.sourceShiftVariants(sourceById(sourceId)||{})}
  function selectShiftVariant(d,variantId){var variant=Core.shiftVariantFor(sourceById(d.sourceId)||{},variantId||'day');d.shiftVariantId=variant.id;d.shiftLabel=variant.label;d.shiftColor=variant.color;d.name=variant.label;d.start=variant.start;d.end=variant.end;return variant}
  function shiftChoicesMarkup(d){return'<div class="calendarShiftChoicesV75" role="group" aria-label="Shift type">'+shiftVariants(d.sourceId).map(function(variant){return'<button type="button" data-shift-variant="'+safe(variant.id)+'" class="'+(variant.id===d.shiftVariantId?'selected':'')+'" aria-pressed="'+(variant.id===d.shiftVariantId)+'" style="--shift-color:'+safe(variant.color)+'"><i></i><span><b>'+safe(variant.shortLabel)+'</b><small>'+safe(timeLabel(variant.start)+'–'+timeLabel(variant.end))+'</small></span></button>'}).join('')+'</div>'}
  ...
  function openWorkSource(){
    ...
    form.onsubmit=function(event){
      ...
      var source={id:id('source'),name:name,color:fields.color.value,shiftVariants:Core.sourceShiftVariants({}),enabled:true,overtimeThreshold:Number(fields.overtimeThreshold.value),createdAt:new Date().toISOString()};
  ...
  function newFlow(kind,key){
    var source=firstSource(),selected=key||activeDate(),variant=Core.shiftVariantFor(source||{},'day');return{kind:kind||'',step:kind?1:0,date:selected,draft:{date:selected,sourceId:source?.id||'',sourceName:source?.name||'Work',shiftVariantId:variant.id,shiftLabel:variant.label,shiftColor:variant.color,name:variant.label,start:variant.start,end:variant.end,pattern:'weekdays',anchor:selected,weeks:2,custom:[true,true,false,true,true,true,true,false,true,true,true,true,false,false],exceptionType:'extra_shift',overtimeMode:'extend',timeOffType:'pto',endDate:selected,holidayName:'Workplace holiday'}}
  }
  ...
  function workStepMarkup(){
    ...
    if(flow.step===1)return sheetHead('Work schedule','Step 1 of 3',true)+progress(1)+'<section class="calendarFormStepV42"><p class="calendarEyebrowV42">SHIFT</p><h3>Which shift do you work?</h3><p>Keep one workplace and choose the shift for this schedule.</p>'+(sources().length?'<label>Workplace<select name="sourceId">'+sourceOptions(d.sourceId)+'</select></label>':'<label>Workplace<input name="sourceName" maxlength="60" value="'+safe(d.sourceName)+'"></label>')+shiftChoicesMarkup(d)+'<div class="calendarFieldGridV42"><label>Starts<input name="start" type="time" value="'+safe(d.start)+'"></label><label>Ends<input name="end" type="time" value="'+safe(d.end)+'"></label></div></section>'+flowActions('Cancel','Next');
  ...
  function renderWorkFlow(root){
    root.querySelector('.calendarSheetBodyV42').innerHTML=workStepMarkup();bindSheetBack(root,function(){if(flow.step===1){flow.kind='';flow.step=0}else flow.step--;renderFlow(root)});rememberFields(root,['start','end','sourceId','sourceName','anchor','weeks']);
    root.querySelectorAll('[data-shift-variant]').forEach(function(button){button.onclick=function(){selectShiftVariant(flow.draft,button.dataset.shiftVariant);renderFlow(root)}});
    var sourceSelect=root.querySelector('[name="sourceId"]');if(sourceSelect)sourceSelect.onchange=function(){flow.draft.sourceId=sourceSelect.value;selectShiftVariant(flow.draft,flow.draft.shiftVariantId);renderFlow(root)};
  ...
  function saveWorkFlow(){
    ...
    var now=new Date().toISOString(),variant=Core.shiftVariantFor(source,d.shiftVariantId),rotation=Core.normalizeRotation({id:id('rotation'),name:d.shiftLabel||variant.label,sourceId:source.id,shiftVariantId:variant.id,shiftLabel:d.shiftLabel||variant.label,shiftColor:d.shiftColor||variant.color,preset:preset,humanPreset:humanPreset,anchor:anchor,pattern:pattern,dayStart:d.start,dayEnd:d.end,nightStart:d.start,nightEnd:d.end,exceptions:{},active:true,createdAt:now,updatedAt:now});saveRotations(rotations().concat(rotation));closeOverlay();refreshCalendar();toast?.((d.shiftLabel||variant.label)+' schedule added for '+source.name)
  }

  function extraMarkup(){
    var d=flow.draft,existing=workRows(d.date).find(function(row){return!row.off});if(existing&&d.exceptionType==='overtime'&&d.overtimeMode==='extend'&&!d.overtimeSeeded){var source=sourceById(existing.sourceId||d.sourceId),variant=Core.shiftVariantFor(source||{},existing.shiftVariantId||Core.shiftVariantForTimes(source||{},existing.start,existing.end).id);d.sourceId=source?.id||d.sourceId;d.shiftVariantId=variant.id;d.shiftLabel=variant.label;d.shiftColor=variant.color;d.start=existing.end||String(existing.time||'').match(/\d\d:\d\d/g)?.[1]||d.start;d.end=Core.timeAfter(d.start,120);d.overtimeSeeded=true}
    return sheetHead('Add to '+dateLabel(d.date,{month:'short',day:'numeric'}),'One-off changes never rewrite your base schedule.',true)+'<section class="calendarFormStepV42"><p class="calendarEyebrowV42">EXCEPTION</p><h3>What changed?</h3><div class="exceptionKindsV42">'+[['extra_shift','Extra shift'],['overtime','Overtime'],['call_in','Call-in'],['swap_shift','Swap shift']].map(function(item){return'<button type="button" data-exception="'+item[0]+'" class="'+(d.exceptionType===item[0]?'selected':'')+'">'+item[1]+'</button>'}).join('')+'</div>'+(d.exceptionType==='overtime'&&existing?'<div class="overtimeModeV42"><label><input type="radio" name="overtimeMode" value="extend" '+(d.overtimeMode==='extend'?'checked':'')+'> Extend existing shift</label><label><input type="radio" name="overtimeMode" value="separate" '+(d.overtimeMode==='separate'?'checked':'')+'> Separate overtime shift</label></div>':'')+'<label>Workplace<select name="sourceId">'+sourceOptions(d.sourceId)+'</select></label>'+shiftChoicesMarkup(d)+'<label>Date<input name="date" type="date" value="'+safe(d.date)+'"></label><div class="calendarFieldGridV42"><label>Starts<input name="start" type="time" value="'+safe(d.start)+'"></label><label>Ends<input name="end" type="time" value="'+safe(d.end)+'"></label></div></section>'+flowActions('Back','Save to '+dateLabel(d.date,{month:'short',day:'numeric'}));
  }
  function renderExtraFlow(root){root.querySelector('.calendarSheetBodyV42').innerHTML=extraMarkup();bindSheetBack(root,function(){flow.kind='';renderFlow(root)});rememberFields(root,['date','start','end','sourceId']);root.querySelectorAll('[data-shift-variant]').forEach(function(button){button.onclick=function(){selectShiftVariant(flow.draft,button.dataset.shiftVariant);flow.draft.overtimeSeeded=false;renderFlow(root)}});var sourceSelect=root.querySelector('[name="sourceId"]');if(sourceSelect)sourceSelect.onchange=function(){flow.draft.sourceId=sourceSelect.value;selectShiftVariant(flow.draft,flow.draft.shiftVariantId);flow.draft.overtimeSeeded=false;renderFlow(root)};root.querySelectorAll('[data-exception]').forEach(function(button){button.onclick=function(){flow.draft.exceptionType=button.dataset.exception;flow.draft.overtimeSeeded=false;renderFlow(root)}});root.querySelectorAll('[name="overtimeMode"]').forEach(function(input){input.onchange=function(){flow.draft.overtimeMode=input.value;flow.draft.overtimeSeeded=false;renderFlow(root)}});root.querySelector('[data-flow-back]').onclick=function(){flow.kind='';renderFlow(root)};root.querySelector('[data-flow-next]').onclick=saveExtra}
  function saveExtra(){var d=flow.draft,source=ensureSource(d.sourceId,d.sourceName),variant=Core.shiftVariantFor(source,d.shiftVariantId),now=new Date().toISOString(),kindLabel={extra_shift:'Extra shift',overtime:'Overtime',call_in:'Call-in',swap_shift:'Swap shift'}[d.exceptionType]||'Extra shift',event={id:id('shift'),kind:'work',date:d.date,title:kindLabel+' · '+variant.shortLabel,start:d.start,end:d.end,overnight:Core.minutes(d.end)<=Core.minutes(d.start),sourceId:source.id,sourceName:source.name,shiftVariantId:variant.id,shiftLabel:variant.label,color:d.shiftColor||variant.color,exception:true,exceptionType:d.exceptionType,overtimeMode:d.exceptionType==='overtime'?d.overtimeMode:'',confidence:{label:'High',score:1,reasons:['Added directly by you']},provenance:{type:'calendar-exception'},createdAt:now,updatedAt:now};saveEvents(events().concat(event));selectedDate=d.date;closeOverlay();refreshCalendar();toast?.(kindLabel+' added as '+variant.label.toLowerCase())}
