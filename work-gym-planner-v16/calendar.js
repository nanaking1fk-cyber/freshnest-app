...
function renderDayCard(){
 ...
 let schedule=`<section class="daySchedule"><div class="daySectionHead"><div><small>DAY PLAN</small><h3>Timeline</h3></div></div>${workRows.map(row=>`<div class="dayScheduleRow ${row.unknown?'unknown':''}" ${row.color?`style="--source-color:${esc(row.color)}"`:''}><span aria-hidden="true">${row.unknown?'?':'W'}</span><div><b>${esc(row.name)}</b><small>${esc(row.time)}</small></div></div>`).join('')||'<div class="dayScheduleEmpty">No work scheduled.</div>'}${wi!=null?`<div class="dayScheduleRow workout"><span aria-hidden="true">↗</span><div><b>${esc(tr)}</b><small>${comp?'Completed':profile()?.trainingMode==='existing'?`${plannedWorkoutTime(k)||'Time flexible'} · your routine`:'Planned around your workday'}</small></div></div>`:'<div class="dayScheduleRow recovery"><span aria-hidden="true">○</span><div><b>Recovery</b><small>No training planned</small></div></div>'}</section>`;
