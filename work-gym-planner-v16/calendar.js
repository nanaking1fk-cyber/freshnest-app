...
function calendarCellDetails(workRows,agenda,gym,k){
 let entries=[];
 if(workRows.length){let row=workRows.find(item=>!item.unknown)||workRows[0];entries.push(`<span class="dayDetail ${row.off?'off':'work'}" style="--detail-color:${esc(row.color||'#58a6ff')}" title="${esc(row.name)}">${esc(row.name)}</span>`)}
