...
function addDayItem(k,item){let all=dayItems();all[k]=[...(all[k]||[]),{id:uid('agenda'),title:item.title.trim(),time:item.time||'',type:item.type==='event'?'event':'todo',done:false,createdAt:new Date().toISOString()}];saveDayItems(all)}
...
function workScheduleRows(k){
 let p=profile(),s=workState(k),rows=[];
 ...
 if(s.variable===true&&p?.variable)rows.push({name:p.variable.name||'Secondary job',time:p.variable.start&&p.variable.end?`${p.variable.start}–${p.variable.end}`:'Scheduled work'});
...
function renderDayCard(){
 ...
 let agendaRows=agenda.map(item=>`<div class="dayAgendaRow ${item.done?'done':''} ${item.recurring?'recurring':''}">${item.type==='todo'&&!item.recurring?`<button class="agendaCheck" data-agenda-toggle="${esc(item.id)}" aria-label="${item.done?'Mark incomplete':'Mark complete'}">${item.done?'✓':''}</button>`:`<span class="agendaEvent" aria-hidden="true">${item.recurring?'↻':'•'}</span>`}<div><b>${esc(item.title)}</b><small>${esc(item.time||(item.type==='todo'?'Any time':'Scheduled item'))}${item.recurring?' · repeats weekly':''}</small></div>${item.recurring?'':`<button class="agendaDelete" data-agenda-delete="${esc(item.id)}" aria-label="Delete ${esc(item.title)}">×</button>`}</div>`).join('');
 let agendaSection=`<section class="dayAgenda"><div class="daySectionHead"><div><small>AGENDA</small><h3>Schedule & to-do</h3></div><span>${agenda.length}</span></div><div class="dayAgendaList">${agendaRows||'<p class="dayScheduleEmpty">Nothing else planned. Add a to-do or schedule item below.</p>'}</div><form class="dayAgendaForm" data-agenda-form><select name="type" aria-label="Item type"><option value="todo">To-do</option><option value="event">Schedule</option></select><input name="title" type="text" required maxlength="100" placeholder="What needs to happen?" aria-label="Item title"><input name="time" type="time" aria-label="Item time"><button class="primary" type="submit">Add</button></form></section>`;
 ...
 let form=$('[data-agenda-form]');if(form)form.onsubmit=event=>{event.preventDefault();let title=form.elements.title.value.trim();if(!title)return;addDayItem(k,{title,type:form.elements.type.value,time:form.elements.time.value});renderCalendar();toast('Added to your calendar')}
}
