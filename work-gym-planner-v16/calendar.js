...
function calendarDisplayMode(){return jget(CALENDAR_DISPLAY_KEY,'compact')==='details'?'details':'compact'}
...
function calendarWorkDisplay(row){return window.WWV25?.workRowDisplay?.(row)||{workplace:row.sourceName||row.name||'Work',shift:row.shiftLabel||'Work shift',time:row.time||''}}
function calendarCellDetails(workRows,agenda,gym,k){
 let entries=[];
 if(workRows.length){let row=workRows.find(item=>!item.unknown)||workRows[0],display=calendarWorkDisplay(row),label=row.off?row.name:[display.workplace,display.shift].filter(Boolean).join(' · ');entries.push(`<span class="dayDetail ${row.off?'off':'work'}" style="--detail-color:${esc(row.color||'#58a6ff')}" title="${esc(label)}">${esc(label)}</span>`)}
...
function renderCalendarWeekRail(){
 ...
 for(let i=0;i<7;i++){
  let current=new Date(start);current.setDate(start.getDate()+i);let k=dkey(current),work=workScheduleRows(k).some(row=>!row.unknown&&!row.off),plans=agendaItemsOn(k).length,training=isScheduled(k)||!!completedOn(k);
...
function renderCalendar(){
 ...
 for(let n=1;n<=days;n++){
  ...
  let workDots=window.WWV25?.workDots?.(workRows)||((workRows.some(row=>!row.unknown&&!row.off)?'<i class="dot blue"></i>':'')),dots='<span class="dayDots">'+workDots+(gym?'<i class="dot green"></i>':'')+(agenda.length?'<i class="dot purple"></i>':'')+(s.kind==='unknown'?'<i class="dot gray"></i>':'')+'</span>',details=calendarCellDetails(workRows,agenda,gym,k);
  let tr=gym?(typeof plannedWorkoutName==='function'?plannedWorkoutName(k,comp?.workoutIndex??projectedWorkoutIndex(k)):(comp?WORKOUTS[comp.workoutIndex].name:WORKOUTS[projectedWorkoutIndex(k)].name)):'No training planned',aria=`${fmt(k)}. ${workRows.length?workRows.map(row=>{let display=calendarWorkDisplay(row);return`${display.workplace} ${display.shift} ${display.time}`}).join('. '):'No work added'}. Training: ${tr}. ${agenda.length} plan ${agenda.length===1?'item':'items'}.${s.kind==='unknown'?' Work schedule needs review.':''}`;
...
function renderDayCard(){
 ...
 let workSources=(window.WWV25?.sources?.()||[]).filter(source=>source.enabled!==false),workSourceOptions=workSources.map(source=>`<option value="${esc(source.id)}">${esc(source.name)}</option>`).join('');
 let tr=wi!=null?(typeof plannedWorkoutName==='function'?plannedWorkoutName(k,wi):WORKOUTS[wi].name):'Recovery / no planned lifting';
 let workCount=workRows.filter(row=>!row.unknown&&!row.off).length,offCount=workRows.filter(row=>row.off).length,workLabel=workCount?`${workCount} work block${workCount===1?'':'s'}`:(offCount?'Off work':unknown?'Work needs review':'No work added');
 ...
 let nutritionDetail=`${t.cal} kcal · ${t.p}g protein${t.c!=null?` · ${t.c}g carbs`:''}${t.f!=null?` · ${t.f}g fat`:''}`;
 let schedule=`<section class="daySchedule"><div class="daySectionHead"><div><small>DAY PLAN</small><h3>Timeline</h3></div></div>${workRows.map((row,index)=>{let display=calendarWorkDisplay(row);return`<div class="dayScheduleRow ${row.unknown?'unknown':''} ${row.off?'off':''} ${row.done?'done':''}" ${row.color?`style="--source-color:${esc(row.color)}"`:''}><span aria-hidden="true">${row.unknown?'?':row.off?'○':(row.done?'✓':'W')}</span><div><b>${esc(row.off?row.name:display.workplace)}</b><small>${esc(row.off?row.time:[display.shift,display.time].filter(Boolean).join(' · '))}${row.done?' · completed':''}</small></div>${!row.unknown?`<span class="dayItemActionsV34">${row.off?'':`<button type="button" data-work-toggle="${index}" aria-label="${row.done?'Mark incomplete':'Mark complete'} ${esc(display.workplace)}">${row.done?'Undo':'Done'}</button>`}<button type="button" data-work-delete="${index}" aria-label="Remove ${esc(display.workplace)} from ${esc(fmt(k))}">Remove</button></span>`:''}</div>`}).join('')||'<div class="dayScheduleEmpty">No work scheduled.</div>'}${wi!=null?`<div class="dayScheduleRow workout"><span aria-hidden="true">↗</span><div><b>${esc(tr)}</b><small>${comp?'Completed':profile()?.trainingMode==='existing'?`${plannedWorkoutTime(k)||'Time flexible'} · your routine`:'Planned around your workday'}</small></div></div>`:'<div class="dayScheduleRow recovery"><span aria-hidden="true">○</span><div><b>Recovery</b><small>No training planned</small></div></div>'}<div class="dayScheduleRow nutrition"><span aria-hidden="true">N</span><div><b>Nutrition target</b><small>${esc(nutritionDetail)}</small></div></div></section>`;
 let agendaRows=agenda.map(item=>{let canEdit=!item.recurring||item.sourceRecurring==='calendar',canComplete=!item.recurring||item.sourceRecurring==='calendar',repeatText=item.sourceRecurring==='calendar'?` · ${recurrenceLabel(item.frequency)}`:(item.recurring?' · repeats weekly':'');return`<div class="dayAgendaRow ${item.done?'done':''} ${item.recurring?'recurring':''}">${item.recurring?`<span class="agendaEvent" aria-hidden="true">↻</span>`:`<span class="agendaEvent" aria-hidden="true">${item.type==='todo'?'✓':'•'}</span>`}<div><b>${esc(item.title)}</b><small>${esc(item.time?item.time+(item.end?'–'+item.end:''):(item.type==='todo'?'Any time':'Scheduled item'))}${item.reminderMinutes?` · reminder ${item.reminderMinutes} min before`:''}${repeatText}</small></div>${canEdit?`<span class="agendaActions">${canComplete?`<button class="agendaDone" data-agenda-toggle="${esc(item.id)}" aria-label="${item.done?'Mark incomplete':'Mark complete'} ${esc(item.title)}">${item.done?'Undo':'Done'}</button>`:''}<button class="agendaEdit" data-agenda-edit="${esc(item.id)}" aria-label="Edit ${esc(item.title)}">Edit</button><button class="agendaDelete" data-agenda-delete="${esc(item.id)}" aria-label="Remove ${esc(item.title)}">Remove</button></span>`:''}</div>`}).join('');
 let agendaSection=`<section class="dayAgenda"><div class="daySectionHead"><div><small>PERSONAL</small><h3>Plans &amp; tasks</h3></div><span>${agenda.length}</span></div><div class="dayAgendaList">${agendaRows||'<p class="dayScheduleEmpty">No personal plans yet.</p>'}</div><button id="dayAddToggleV33" class="dayAddToggleV33" type="button"><span>+</span> Add item</button><form class="dayAgendaForm" data-agenda-form data-kind="todo" hidden><input name="editId" type="hidden"><div class="dayAgendaPrimaryV33"><select name="type" aria-label="Item type"><option value="todo">To-do</option><option value="event">Event</option><option value="payday">Payday</option><option value="off">Off work</option></select><input name="title" type="text" required maxlength="100" placeholder="What needs to happen?" aria-label="Item title"></div><label class="dayAgendaSourceV37">Work source<select name="sourceId" aria-label="Work source">${workSourceOptions}</select></label><div class="dayAgendaMetaV33"><input name="time" type="time" aria-label="Item time"><input name="end" type="time" aria-label="End time"><select name="reminderMinutes" aria-label="Reminder"><option value="0">No reminder</option><option value="15">15 min reminder</option><option value="30">30 min reminder</option><option value="60">1 hour reminder</option><option value="720">12 hour reminder</option></select><select name="repeat" aria-label="Repeat"><option value="none">Does not repeat</option><option value="weekly">Every week</option><option value="biweekly">Every 2 weeks</option><option value="monthly">Every month</option><option value="yearly">Every year</option></select></div><div class="dayAgendaSubmitV33"><button name="cancelEdit" type="button">Cancel</button><button class="primary" type="submit">Add</button></div></form></section>`;
 ...
 let form=$('[data-agenda-form]');if(form){
  let setKind=kind=>{form.dataset.kind=kind;if(kind==='payday'){if(!form.elements.title.value)form.elements.title.value='Payday';if(form.elements.repeat.value==='none')form.elements.repeat.value='biweekly'}else if(kind==='off'){form.elements.title.value='Off work'}},reveal=()=>{form.hidden=false;$('dayAddToggleV33').hidden=true;setKind(form.elements.type.value);requestAnimationFrame(()=>form.elements.type.value==='off'?form.elements.sourceId.focus():form.elements.title.focus())};
  form.elements.type.onchange=()=>setKind(form.elements.type.value);
  $('dayAddToggleV33').onclick=reveal;$('dayCardAddV33').onclick=reveal;
  $$('[data-agenda-edit]').forEach(button=>button.onclick=()=>{let item=agendaItemsOn(k).find(x=>x.id===button.dataset.agendaEdit);if(!item)return;reveal();form.elements.editId.value=item.sourceRecurring==='calendar'?`recurring:${item.seriesId}`:item.id;form.elements.type.value=item.category==='payday'?'payday':(item.type||'todo');form.elements.title.value=item.title||'';form.elements.time.value=item.time||'';form.elements.end.value=item.end||'';form.elements.reminderMinutes.value=String(item.reminderMinutes||0);form.elements.repeat.value=item.sourceRecurring==='calendar'?(item.frequency||'monthly'):'none';setKind(form.elements.type.value);form.querySelector('[type="submit"]').textContent='Save'});
  form.elements.cancelEdit.onclick=()=>renderCalendar();
  form.onsubmit=event=>{event.preventDefault();let kind=form.elements.type.value;if(kind==='off'){if(!form.elements.sourceId.value){toast('Add a work source before marking an off day');return}if(window.WWV25?.addOffDay?.(k,form.elements.sourceId.value)){toast('Off day added to your calendar');renderCalendar()}return}let title=form.elements.title.value.trim();if(!title)return;if(kind==='payday'&&form.elements.repeat.value==='none'){toast('Payday must repeat. Choose weekly, every 2 weeks, monthly or yearly.');form.elements.repeat.focus();return}let payload={title,type:kind==='payday'?'event':kind,category:kind,time:form.elements.time.value,end:form.elements.end.value,reminderMinutes:+form.elements.reminderMinutes.value||0,frequency:form.elements.repeat.value},editId=form.elements.editId.value,isRecurring=editId.startsWith('recurring:');if(isRecurring){let id=editId.slice('recurring:'.length);if(payload.frequency==='none'){deleteRecurringCalendarItem(id);addDayItem(k,payload);toast('Repeating event changed to a one-time item')}else{patchRecurringCalendarItem(id,payload);toast('Repeating event updated')}}else if(editId){if(payload.frequency==='none'){patchDayItem(k,editId,payload);toast('Calendar item updated')}else{deleteDayItem(k,editId);addRecurringCalendarItem(k,payload);toast('Calendar item now repeats')}}else if(payload.frequency!=='none'){addRecurringCalendarItem(k,payload);toast(`${recurrenceLabel(payload.frequency)} added to your calendar`)}else{addDayItem(k,payload);toast('Added to your calendar')}renderCalendar()}
 }
}
