...
function ensureCommercialProfileUI(){
 let intro=$('#onboardingIntro');if(intro){let b=intro.querySelector('b'),p=intro.querySelector('p');if(b)b.textContent='Set up Work + Workout for your own schedule, training and body goals.';if(p)p.textContent='Your plan adapts to your work cycle, recovery, completed workouts and nutrition. Data stays on this device unless you choose to export or sync it.';let francis=$('#useFrancis');if(francis){francis.style.display='none';francis.setAttribute('aria-hidden','true')}let custom=$('#useCustom');if(custom)custom.textContent='Start my setup';if(!$('#commercialTemplates')){let box=document.createElement('div');box.id='commercialTemplates';box.className='commercialTemplates';box.innerHTML='<button type="button" data-commercial-template="standard"><b>Standard work week</b><small>Mon–Fri style schedule</small></button><button type="button" data-commercial-template="rotating"><b>Rotating / shift work</b><small>Build a repeating 7–28 day cycle</small></button><button type="button" data-commercial-template="variable"><b>Variable monthly schedule</b><small>Enter or scan each month</small></button><button type="button" data-commercial-template="none"><b>No work schedule</b><small>Plan training from recovery only</small></button>';intro.appendChild(box)}}
 ...
 let rot=$('#rotationGrid');if(rot&&!$('#rotationLength')){let h=rot.previousElementSibling;let wrap=document.createElement('div');wrap.className='commercialSetupGrid';wrap.innerHTML='<label>Repeating cycle length<select id="rotationLength"><option value="7">7 days</option><option value="8">8 days</option><option value="10">10 days</option><option value="14">14 days</option><option value="21">21 days</option><option value="28">28 days</option></select></label><label>Planned lifting days / week<select id="trainingDaysPerWeek"><option value="2">2 days</option><option value="3">3 days</option><option value="4">4 days</option></select></label><label>Offer optional workouts on workdays<select id="singleJobTraining"><option value="yes">Yes</option><option value="no">No</option></select></label><label>Equipment access<select id="equipmentMode"><option value="full">Full commercial gym</option><option value="basic">Basic gym</option><option value="home">Home / minimal equipment</option></select></label>';if(h)h.insertAdjacentElement('afterend',wrap);else rot.insertAdjacentElement('beforebegin',wrap)}
...
function addCommercialProductUI(){
 let more=$('#page-more .menuCards');if(more&&!$('#commercialPrivacy')){let p=document.createElement('button');p.id='commercialPrivacy';p.innerHTML='<span>🔒</span><div><b>Privacy & terms</b><small>How your schedule, health and training data are handled</small></div><i>›</i>';p.onclick=()=>location.href=new URL('./privacy.html',location.href).href;more.appendChild(p)}
