...
(function schedulePlatformV25(){
  ...
  function proposalCalendarChip(item){
    var source=sourceById(item.sourceId)||{},time=item.kind==='off'?'Protected off day':(item.start?(formatTime(item.start)+(item.end?'–'+formatTime(item.end):'')):'Any time'),workSource=item.kind==='work'||item.kind==='off'?'<input type="hidden" value="'+safe(item.sourceId)+'" data-proposal-source="'+safe(item.id)+'">':'';
