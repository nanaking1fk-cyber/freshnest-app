...
(function schedulePlatformV25(){
  ...
  var proposals=[],proposalConflicts={},rosterReview=null,proposalParseNotice='',mounting=false,sourceEditId='',shiftPickerActive=false,shiftPickerDates=new Set(),shiftPickerMode='work';
  ...
  function workRowsOn(key){
    ...
    off.forEach(function(event){var source=sourceById(event.sourceId)||{};generated.push(decorateWorkRow({name:(source.name||'Work')+' · Off',time:'Off work',color:source.color||Core.COLORS[0],sourceId:event.sourceId,eventId:event.id,off:true},key))});
    return generated;
  }
  ...
  function renderShiftPicker(){
    ...
    document.getElementById('cancelShiftPickerV35').onclick=stopShiftPicker;document.getElementById('shiftPickerModeV35').onchange=function(){shiftPickerMode=this.value;renderShiftPicker()};document.getElementById('reviewPickedShiftsV35').onclick=reviewPickedShifts;
  }
  function reviewPickedShifts(){
    ...
    var mode=document.getElementById('shiftPickerModeV35')?.value||shiftPickerMode,start=document.getElementById('shiftPickerStartV35')?.value||'07:00',end=document.getElementById('shiftPickerEndV35')?.value||'19:00',seriesId=makeId('series'),existing=existingForReview();
    proposals=[...shiftPickerDates].sort().map(function(key){var off=mode==='off';return{id:makeId('proposal'),kind:off?'off':'work',date:key,title:off?(source.name||'Work')+' · Off':source.name||'Work shift',start:off?'':start,end:off?'':end,overnight:!off&&Core.minutes(end)<=Core.minutes(start),sourceText:'Selected directly on calendar',sourceType:'calendar',sourceId:source.id,seriesId:seriesId,series:true,needsReview:false,confidence:{label:'High',score:1,reasons:['Date selected directly by you']}}});proposalConflicts=Core.detectConflicts(proposals,existing);proposalParseNotice=mode==='off'?'These dates will be protected as off work for this source. The off-day override is reversible and does not delete the underlying rotation.':'You selected these dates directly. Check the calendar preview and approve them when everything looks right.';
    shiftPickerActive=false;shiftPickerDates=new Set();document.body.classList.remove('calendarShiftPickingV35');document.getElementById('shiftPickerBarV35')?.remove();selectTab('add',true);renderTrustedReview();
  }
  ...
  function proposalCalendarChip(item){
    var source=sourceById(item.sourceId)||{},time=item.kind==='off'?'Protected off day':(item.start?(formatTime(item.start)+(item.end?'–'+formatTime(item.end):'')):'Any time'),workSource=item.kind==='work'||item.kind==='off'?'<input type="hidden" value="'+safe(item.sourceId)+'" data-proposal-source="'+safe(item.id)+'">':'',dateControl=item.needsReview?'<label class="proposalCalendarDateV36"><span>Confirm date</span><input type="date" value="'+safe(item.date)+'" data-proposal-date="'+safe(item.id)+'" data-proposal-calendar-date aria-label="Confirm date for '+safe(item.title)+'"></label>':'<input type="hidden" value="'+safe(item.date)+'" data-proposal-date="'+safe(item.id)+'">';
    return'<div class="proposalCalendarChipV25 '+safe(item.kind)+' '+(item.needsReview?'needsReview ':'')+(proposalConflicts[item.id]?'hasConflict':'')+'" style="--proposal-color:'+safe(source.color||Core.COLORS[0])+'"><label class="proposalCalendarSelectV36"><input type="checkbox" '+(item.selected===false?'':'checked')+' data-proposal-check="'+safe(item.id)+'" aria-label="Include '+safe(item.title)+' on '+friendlyDate(item.date)+'"><span><b>'+safe(item.title)+'</b><small>'+safe(time)+(item.needsReview?' · confirm date':'')+(proposalConflicts[item.id]?' · conflict':'')+'</small></span></label>'+dateControl+workSource+'</div>';
  }
  ...
  function saveRotationFromForm(event){event.preventDefault();var preset=document.getElementById('rotationPresetV25').value,custom=document.getElementById('rotationPatternV25').value,pattern=preset==='custom'?Core.parsePattern(custom):Core.patternFromPreset(preset),anchor=document.getElementById('rotationAnchorV25').value;if(preset==='alternating_weekends'||preset==='third_weekend')anchor=weekendRotationAnchor(anchor);if(!pattern.length){toast('Add at least one D, N or O to the custom cycle');return}var now=new Date().toISOString(),rotation=Core.normalizeRotation({id:makeId('rotation'),name:document.getElementById('rotationNameV25').value.trim(),sourceId:document.getElementById('rotationSourceV25').value,preset:preset,anchor:anchor,pattern:pattern,dayStart:document.getElementById('rotationDayStartV25').value,dayEnd:document.getElementById('rotationDayEndV25').value,nightStart:document.getElementById('rotationNightStartV25').value,nightEnd:document.getElementById('rotationNightEndV25').value,exceptions:{},active:true,createdAt:now,updatedAt:now});saveRotations(rotations().concat(rotation));renderRotations();renderCalendar();toast((preset==='alternating_weekends'||preset==='third_weekend')?'Weekend rotation saved from '+friendlyDate(anchor):'Rotation saved with no end date')}
