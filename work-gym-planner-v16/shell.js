...
(function setupV16Shell(){
 ...
 const old=document.getElementById('backupDialog');if(old){old.outerHTML=`<div id="backupDialog" class="modal" role="dialog" aria-modal="true" aria-labelledby="backupTitle"><div class="sheet"><div class="sheetHandle"></div><div class="sheetHead"><h2 id="backupTitle">Data & backup</h2><button data-close="backupDialog">Done</button></div><div class="card noMargin"><h3>Secure backup</h3><p>Protect your planner with a passphrase. Save the file somewhere private and keep the passphrase separately. Work hours and pay have their own export in Calendar.</p><label for="backupPass">Backup passphrase</label><input id="backupPass" class="fullInput" type="password" autocomplete="new-password" placeholder="Use a memorable phrase (12+ characters)"><div class="twoButtons"><button id="exportEncrypted" class="primary">Encrypted download</button><button id="shareEncrypted">Share / save to Files</button></div><p id="backupStatus" class="statusText" role="status"></p></div><div class="card"><h3>Plain backup</h3><p>This file is not password-protected. It contains your planner and health records; share it only with someone you trust.</p><button id="exportData" class="wideBtn">Export plain JSON</button></div><div class="card"><h3>Restore / migrate</h3><label class="captureBtn">Choose backup file<input id="importFile" type="file" accept="application/json,.json"></label><p id="importStatus" class="statusText" role="status"></p><button id="restoreSnapshot" class="wideBtn">Restore last automatic local snapshot</button></div><div class="card"><h3>Diagnostics</h3><button id="exportDiagnostics" class="wideBtn">Export diagnostics</button></div><div class="card dangerZone"><h3>Delete local data</h3><label for="deleteConfirm">Confirm local deletion</label><input id="deleteConfirm" class="fullInput" placeholder="Type DELETE"><button id="deleteAllData" class="danger wideBtn">Delete all planner data</button></div></div></div>`}
 const about=document.querySelector('#aboutDialog .card');if(about)about.innerHTML='<p><b>Version:</b> 30.1.31</p><p>Adaptive work, recovery, training and nutrition planning with private account sync and review-before-save schedule intake.</p><p>The native iPhone and Android apps can read daily steps with permission. Websites and PWAs cannot access HealthKit or Health Connect directly.</p>';
})();
