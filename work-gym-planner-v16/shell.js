...
(function setupV16Shell(){
 ...
 if(cards&&!cards.querySelector('[data-open="health"]')){
   ...
   const box=document.createElement('div');box.innerHTML=`<button data-open="health"><span>❤️</span><div><b>Health & recovery</b><small>Apple Health import, sleep, steps, resting HR</small></div><i>›</i></button><button data-open="reminders"><span>🔔</span><div><b>Reminders & Calendar</b><small>Workout alerts and iOS Calendar export</small></div><i>›</i></button><button data-open="cloud"><span>☁️</span><div><b>Private cloud sync</b><small>Encrypted WebDAV / Nextcloud sync</small></div><i>›</i></button>`;
   [...box.children].forEach(x=>cards.insertBefore(x,backup));
 }
 const modalHTML=`
 <div id="healthDialog" class="modal" role="dialog" aria-modal="true" aria-labelledby="healthTitle"><div class="sheet"><div class="sheetHandle"></div><div class="sheetHead"><h2 id="healthTitle">Health & recovery</h2><button data-close="healthDialog">Done</button></div>
  <div class="card noMargin"><h3>Apple Health import</h3><p>Export your Health data on iPhone, unzip it in Files, then choose <b>export.xml</b>. The file is parsed locally on this device.</p><label class="captureBtn">Choose Apple Health export.xml<input id="healthXml" type="file" accept="text/xml,.xml"></label><p id="healthImportStatus" class="statusText"></p></div>
 ...
 const about=document.querySelector('#aboutDialog .card');if(about)about.innerHTML='<p><b>Version:</b> 30.1.29</p><p>Adaptive work, recovery, training and nutrition planning with private account sync and review-before-save schedule intake.</p><p>Apple Health files are parsed locally. Direct HealthKit access is not available to Safari/PWA apps.</p>';
})();
