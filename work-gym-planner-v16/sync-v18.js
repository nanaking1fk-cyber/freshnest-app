...
(function(A){
 ...
 async function push(){if(pushing||!dirty||window.navigator?.onLine===false||!A.session||!A.cloudStateReady||typeof A.pushState!=='function')return;pushing=true;const sending=generation;try{let sent=await A.pushState({quiet:true});if(sent!==false){dirty=generation!==sending;lastPush=Date.now()}}catch(e){console.warn('Account autosync deferred:',e.message)}finally{pushing=false}}
