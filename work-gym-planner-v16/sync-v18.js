...
(function(A){
 ...
 async function push(){if(pushing||!dirty||!A.session||typeof A.migrateThisDevice!=='function')return;pushing=true;try{await A.migrateThisDevice();dirty=false;lastPush=Date.now()}catch(e){console.warn('Account autosync deferred:',e.message)}finally{pushing=false}}
 ...
 if(typeof oldSet==='function'&&!oldSet.__sync18){let wrapped=function(key,val){let r=oldSet(key,val);if(A.session&&key!==AUTH18)A.queueSync();return r};wrapped.__sync18=true;window.jset=wrapped}
