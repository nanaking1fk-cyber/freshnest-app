...
function dueWorkoutIndex(before=null){return workoutHistory().filter(x=>x.completed&&(!before||x.date<before)).length%WORKOUTS.length}
function completedOn(k){return workoutHistory().find(x=>x.completed&&x.date===k)||null}
function trainingStressPenalty(k){let h=workoutHistory().filter(x=>x.completed&&x.date<k).sort((a,b)=>b.date.localeCompare(a.date))[0];if(!h)return 0;let gap=diffDays(k,h.date);if(gap<=1)return -100;if(gap===2&&WORKOUTS[h.workoutIndex]?.stress==='lower')return -2;return 0}
...
function suggestedTrainingDates(start,end){
 ...
 for(let wk=firstMon;wk<=last;wk=addDays(wk,7)){
   ...
   if(picked.length<3){/* never force adjacent days; fewer sessions is safer */}
...
function freezePastSnapshots(){if(!profile())return;let s=snapshots(),today=dkey(),dates=canonicalScheduleThrough(today).filter(k=>k>=addDays(today,-35)&&k<=today);for(const k of dates)if(!s[k])s[k]={planned:true,workoutIndex:completedOn(k)?.workoutIndex??projectedWorkoutIndex(k),frozenAt:new Date().toISOString()};for(const h of workoutHistory())if(h.completed&&!s[h.date])s[h.date]={planned:true,workoutIndex:h.workoutIndex,frozenAt:h.completedAt};saveSnapshots(s)}
