(async()=>{try{
  ...
  let h=await fetch('../work-gym-planner-v15/index.html?v=30.1.31-hero72'+(authCallback?'&auth=callback':''),{cache:authCallback?'no-store':'force-cache'}).then(r=>{if(!r.ok)throw Error('base');return r.text()});
